...
**Upgrading an existing instance to a version that adds a new demo locale** (as German and French did here) has the same gap for that locale specifically: `demoEmailFor()` resolves it to a real account email unconditionally the moment the new frontend build is live, but that account doesn't exist in the database until `app:reset-demo-data` next runs — up to an hour away on the default cron schedule. Run it once by hand right after the redeploy (same command as the cron line above) rather than waiting for the next tick, the same way first enabling `DEMO_MODE` already calls for.
