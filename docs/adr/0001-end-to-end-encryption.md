...
- Losing a password without a fresh keypair genuinely loses access to old encrypted content sealed under the discarded key — this is why password reset (`CLAUDE.md`, "Password reset, Part 1/2") issues a fresh keypair and re-shares anketa keys via the counterpart, rather than "recovering" the old one, which is cryptographically impossible without escrowing keys server-side.
