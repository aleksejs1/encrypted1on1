...
- **Change password.** Unlike the forgot-password flow above, this is for someone who still remembers their current password and just wants a new one — it re-wraps the *same* existing keypair rather than generating a fresh one, so nothing becomes unreadable and no counterpart ever needs to re-share anything. That includes your own unpublished drafts, whose key comes from the keypair, not the password. After a forgotten-password *reset*, a draft saved before the reset can't be opened any more. It shows up as an empty draft with a notice instead of breaking the page. Requires entering the current password first, the same way any sensitive account action does.
...
- **Export your data** downloads everything currently visible to you — your own answers (including an unpublished draft, if you have one — flagged `myDraftUnreadable` instead if it can no longer be opened, e.g. after a forgotten-password reset), any counterpart's published answers, comments, outcomes, and goals, plus your private notes in a separate `privateNotes` list (as last saved, and flagged `unreadable` instead if they can no longer be opened) — as a single JSON file, decrypted entirely in the browser. Nothing new is computed on the server for this; it's the same data the app already lets you see, fetched in one batch and gathered into a file instead of being displayed.
...
Each side answers their own set of questions privately — an employee side (mood, workload, feelings, growth, friction, achievements, things to discuss) and a manager side (how the period went, feedback, support, the employee's achievements worth recognizing, things to discuss). See [methodology.md](methodology.md#what-each-question-is-asking-and-why) for what each one is actually asking. Answers autosave as a draft every second of inactivity, encrypted even before publishing with a key only you have (see [encryption.md](encryption.md#draft-key)).

**Private notes.** A panel on each anketa holds notes only you can read: your counterpart can't, and neither can the server or your company's admins ([encryption.md](encryption.md#private-notes)). Use it for anything you want to remember about the meeting that isn't for sharing. The notes autosave as you type, stay editable after the anketa is archived, and never become part of your published answers. On a wide screen they're a column beside the form that stays in view while you scroll, and on a narrow one a card under the meeting header. **Hide notes** takes the text off the screen, for screen sharing. If two tabs change the same notes, you're asked which version to keep, with keeping both as the default. A forgotten-password reset makes existing notes unreadable, so export your data first if you need to keep a copy.
...
- If it hasn't happened yet, either side sees an "overdue" state with two options: **reschedule** (push the date, same anketa) or **cancel as missed** (archives this cycle as missed; the next anketa is still auto-created the same way as below, unless "don't create the next meeting" is checked).
- Otherwise, either participant can **archive** it once both sides feel it's done. Archiving does two things: it closes this cycle, and — unless "don't create the next meeting" is checked — it immediately creates the *next* anketa for the same pair, on the same periodicity, with a fresh encryption key generated and sealed the same way as the very first one. The form's **Next meeting type** sets the new anketa's meeting type. It starts on a regular check-in, since no other type repeats by default, and can be changed to any type; that type's own next meeting is back to a regular check-in unless someone picks it again. "Cancel as missed" uses the same choice. Any unchecked items from the outcomes list carry forward into the new anketa automatically, still encrypted, still requiring both participants' keys to read. Open goals carry forward too (their plaintext row is copied so status/history persists across cycles).
...
- A pair can have a second open anketa alongside its regular one, e.g. a hand-created ad-hoc one using a different meeting template. Such an anketa is a *one-off*. It starts without the carried-forward goals and outcomes, since the already-open one has them. Goals and outcomes added inside it don't carry on to the pair's regular meetings either. Archiving it never auto-creates a next anketa, so the pair's chain never forks into two. The archive form says so instead of offering the next-meeting controls. See [the decision record](decisions/2026-09-23-one-open-anketa-chain-per-pair.md).
