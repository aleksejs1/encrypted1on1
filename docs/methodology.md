...
- **Feelings** — a short checklist (excited, anxious, confident, overwhelmed, motivated, frustrated) plus an optional note. More specific than "mood," and easier to select from a list than to put into words unprompted — it exists so a real emotional signal doesn't get lost simply because naming it felt awkward.
