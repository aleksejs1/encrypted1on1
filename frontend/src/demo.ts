 ...
 *
 * Every current UI locale has a demo pair now — German and French (issues
 * #34/#46) were the last two to add translated demo narrative content,
 * alongside their interface translations. A future locale added to
 * SUPPORTED_LOCALES without demo content yet should carry an explicit
 * `null` below (rather than being left out of the map) so the map stays a
 * compile error, not a silent gap, until it's filled in — demoEmailFor()
 * falls back to the English demo account for any `null` entry rather than
...
const DEMO_EMPLOYEE_EMAILS: Record<SupportedLocale, string | null> = {
  ...
  de: 'demo-employee-de@example.com',
  fr: 'demo-employee-fr@example.com',
};
