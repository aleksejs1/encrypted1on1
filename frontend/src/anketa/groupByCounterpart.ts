export interface CounterpartGroup<T extends { counterpartId: string; counterpartEmail: string }> {
 ...
 */
export function groupByCounterpart<T extends { counterpartId: string; counterpartEmail: string }>(
  anketas: T[],
): CounterpartGroup<T>[] {
  ...
  for (const anketa of anketas) {
    ...
    if (!group) {
      group = { counterpartId: anketa.counterpartId, counterpartEmail: anketa.counterpartEmail, anketas: [] };
